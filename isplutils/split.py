...
def get_split_df(df: pd.DataFrame, dataset: str, split: str) -> pd.DataFrame:
    if dataset == 'dfdc-35-5-10':
        ...
        random_youtube_videos = np.random.permutation(
            df[df['quality'] == crf]['video'].unique())
        train_orig = random_youtube_videos[:2100]
        val_orig = random_youtube_videos[2100:2100 + 350]
        test_orig = random_youtube_videos[2100 + 350:]
        if split == 'train':
        ...
        raise NotImplementedError('Unknown dataset: {}'.format(dataset))
    return split_df.drop_duplicates()
